...
const Dashboard = () => {
  ...
  // Get recommended destinations (top 4 cities from mockCities)
  const recommendedCities = mockCities.slice(0, 4);

  // Calculate budget stats
  const totalSpent = userTrips.reduce((sum, trip) => sum + (trip.budget?.spent || 0), 0);
    ...
    <div className="space-y-8">
      ...
      {upcomingTrips.length > 0 && (
        <div className="card p-6 border-l-4 border-l-primary">
          ...
          {upcomingTrips[0] && (
            <Link to={`/trips/${upcomingTrips[0].id}`} className="flex items-center justify-between group">
              <div>
